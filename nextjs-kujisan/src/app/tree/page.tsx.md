...
import ReactFlow, { 
  ...
  Node,
  Edge,
  useReactFlow // <--- NEW: Import hook for camera control
} from 'reactflow';
...
function AncestryCanvas() {
  ...
  const { fitView } = useReactFlow(); // <--- NEW: Camera control
  ...
  useEffect(() => {
    const initTree = async () => {
      ...
      // Initial center after slight delay to allow rendering
      setTimeout(() => fitView({ duration: 0, padding: 0.2 }), 100);
    };
    initTree();
  }, [fitView, nodesMap, edgesMap, refreshLayout]);

  // Ensure nodes keep their handlers
  useEffect(() => {
     nodes.forEach(node => {
  ...
  return (
    <div className="w-full h-[calc(100vh-64px)] bg-stone-50 relative group/canvas">
      ...
      >
        ...
        {/* Controls moved up to accommodate mobile hint */}
        <Controls showInteractive={false} className="mb-16 md:mb-0" />
      </ReactFlow>

      {/* --- FEATURE 1: LEGEND (Top Right) --- */}
      <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm p-3 rounded-2xl shadow-sm border border-stone-100 text-xs font-medium space-y-2 opacity-50 hover:opacity-100 transition-opacity">
